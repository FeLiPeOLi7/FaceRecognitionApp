...
def process_frame_bytes(image_bytes: bytes, sid: str, resize_scale: float = 0.20) -> bytes:
    """Processes a frame safely and returns marked JPEG bytes."""
    ...
    try:
        # Segurança: Garante que os bytes recebidos da rede formam uma imagem válida
        img = Image.open(io.BytesIO(image_bytes)).convert("RGB")
        frame = cv2.cvtColor(np.array(img), cv2.COLOR_RGB2BGR)
    except Exception:
        # Se o pacote chegou corrompido ou incompleto, retorna uma matriz vazia em JPEG de segurança
        blank_frame = np.zeros((480, 640, 3), dtype=np.uint8)
        cv2.putText(blank_frame, "Erro: Frame Incompleto", (50, 240), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
        _, jpg = cv2.imencode('.jpg', blank_frame)
        return jpg.tobytes()
